...
typedef struct
{
	...
	_Bool SendData20ms_Flag;//20ms数据上传标志
	u8    FrameLength;		//帧长度
	u16   CMD;				//数据命令
	u8    DataNum;			//命令个数
	s16   CheckSum;			//校验码
	u8    Product_Type; 	//产品类型
...
typedef enum 
{
	OFFSendData20ms=0x8000, //关闭20ms数据上传
	SWhellRollSpeed, 		//轮子转速
	SWhellSpeed,			//轮子速度
	STurningRadius,     	//轮子拐弯半径
	SWhellAcceleration,     //轮子加速度
	SChassisAttitude,  		//底盘姿态
	SPSRawData,				//PS原始数据
	ONSendData20ms,			//上传数据命令
